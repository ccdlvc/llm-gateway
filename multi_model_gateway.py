...
class MultiModelGateway(LLMGateway):
    ...
    def _detect_task_type(self, prompt: str) -> str:
        ...
        code_keywords = [
            "function", "def ", "class ", "import ", "def ", "async def",
            "react", "vue", "typescript", "python", "java", "rust", "go",
    ...
    def _get_best_model_for_task(self, task_type: str) -> Optional[str]:
        ...
        task_model_map = {
            ...
            "general_chat": "claude-3-haiku",  # cheapest for general tasks
        }
...
def demo_multi_model():
    ...
    for model_name in gateway.models:
        router = gateway.model_router[model_name]
        stats = router.router.get_account_stats(router.router.accounts["acc-1"].id)
        if stats:
