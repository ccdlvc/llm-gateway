//! LLM Gateway Core - Rust implementation
//! 
//! This module provides the core routing engine that can be used by:
//! - The Zed plugin (via lib.rs)
//! - A standalone server (via main.rs with tokio runtime)
//! - Other applications via the public API
...
pub struct AccountRouter {
    ...
    cost_trackers: HashMap<String, f64>,
    request_counts: HashMap<String, Vec<f64>>,
}
    ...
    pub fn update_request_result(&mut self, account_id: &str, result: &serde_json::Value) {
        if let Some(account) = self.accounts.get(account_id) {
            ...
            if let Some(counts) = self.request_counts.get_mut(account_id) {
                counts.push(Utc::now().timestamp_secs() as f64);
                // Keep only last 60 seconds
                let cutoff = Utc::now().timestamp_secs() - 60;
                counts.retain(|&t| t >= cutoff as f64);
            }
        }
    }
